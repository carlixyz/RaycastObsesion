...
PixelBuffer ground(HALF_SCR_W, HALF_SCR_H >> 1);						// Main Window PreBuffer
PixelBuffer textureBrick(64, 64); //red bricks
...
void main()
{
	...
	for (int x = 0, texWidth = textureBrick.Width; x < texWidth; x++)
	for (int y = 0, texHeight = textureBrick.Height; y < texHeight; y++)
	{
		//int xorcolor = (x * 256 / texWidth) ^ (y * 256 / texHeight);
		int ycolor = y * 256 / texHeight;
		//int xycolor = y * 128 / texHeight + x * 128 / texWidth;
		//texture[3][texWidth * y + x] = xorcolor + 256 * xorcolor + 65536 * xorcolor; //xor greyscale
		textureBrick[texWidth * y + x] = 0x000000FF * (x % 16 && y % 16); //red bricks
	}

	//generate ground
	for (int x = 0, texWidth = ground.Width; x < texWidth; x++)
	for (int y = 0, texHeight = ground.Height; y < texHeight; y++)
	{
		int ycolor = y * 128 / texHeight;		// 256
		ground[ground.Width * y + x] = 65536 * ycolor; //red gradient
	}
	...
	while (true)
	{
		...
		gfx::add(ground, backBuffer, 0, HALF_SCR_H >> 1);
		...
		//gfx::add(textureBrick, backBuffer, 100, 50);
...
void Entity::CastSingleRay(float rayAngle, int stripIdx)
{
	...
	if (dist)
	{
		...
		if (!EyeFish)
			...
			//dist = dist * cos((Player.Rot - rayAngle) * 2);
			//dist = dist + sin((Player.Rot - rayAngle);

		// now calc the position, height and width of the wall strip

		// "real" wall height in the game world is 1 unit, the distance from the player to the screen is viewDist,
		// thus the height on the screen is equal to wall_height_real * viewDist / dist
		int height = (int)(viewDist / dist);	//(ColumnHeight = 1 * viewDist / dist)  
		...
		//where exactly the wall was hit	(textureX)
		if (wallType != 1)
		{
			//x coordinate on the texture
			int texX = int(textureX * double(textureBrick.Width));
			if (side == 0 && xHit > 0) texX = textureBrick.Width - texX - 1;
			if (side == 1 && yHit < 0) texX = textureBrick.Width - texX - 1;

			for (int y = drawStart; y < drawEnd; y++)
			{
				int d = y * 256 - HALF_SCR_H * 128 + height * 128;
				int texY = ((d * textureBrick.Height) / height) / 256 ;
				
				if (!textureBrick.onArea(texX, texY ))
					return;
				
				int pcolor = textureBrick[textureBrick.Height * texY + texX];
				//make color darker for y-sides: R, G and B byte each divided through two with a "shift" and an "and"
				if (side == 1) pcolor = (pcolor >> 1) & 8355711;
				//backBuffer[y*HALF_SCR_W + (stripIdx*stripWidth) ] = pcolor;
				//backBuffer[y*HALF_SCR_W + (stripIdx*stripWidth)+1 ] = pcolor;
				backBuffer.SetPixel((stripIdx*stripWidth), y, pcolor);
				backBuffer.SetPixel((stripIdx*stripWidth)-1, y, pcolor);
			}
			return;
		}
...
void Entity::UpdateInput(float timeStep)
{
	...
	if (MainApp.KeyPressed(Key::D))		// Add Strafe to Right (Move Transversally to your Front Axis)
	{
		dX -= sinR * MoveSpeed;
		dY += cosR * MoveSpeed;
	}

	if (MainApp.KeyPressed(Key::A))		 //Add Strafe to Left
	{
		dX += sinR * MoveSpeed;
		dY -= cosR * MoveSpeed;
	}
	...
	if (MainApp.KeyPressed(Key::Delete)) // Simulate LookDown
		HorizontLine = 0;
}
